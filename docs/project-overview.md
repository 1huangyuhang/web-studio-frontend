...
本文档面向需要快速理解 **Redwood / 林之源** 全栈仓库的开发者与评审者，概括当前代码结构、技术选型、运行时拓扑、认证与 API 分层，以及前后端数据如何流动。细节实现以源码为准；契约与响应形态另见 [api-response-conventions.md](./api-response-conventions.md)。
...
```text
前端/   （仓库根；包名 enterprise-frontend-framework）
├── src/                    # 官网应用
...
1. **API Key**：请求头 `x-api-key` 与后端 `API_KEY` 一致（与前端构建时 `VITE_API_KEY` 对齐，见根 `README`）。
...
**`/api/categories`**：分类列表等（按路由文件实现为准）。
...
后端 **成功响应** 并非单一信封：登录、注册、列表、详情在各控制器中形态略有差异；**错误**经 `errorHandler` 时包含 `message`、`statusCode`、`requestId`、`errorRef` 等，便于与 `shared/errorTracing` 对齐。详见 [api-response-conventions.md](./api-response-conventions.md)。
...
- **实时**：统计等可通过 Socket 与后端同步（以当前 `useManagementStatsSync` 等实现为准）。
...
常用命令见仓库根目录 [README.md](../README.md)，其中包括：

- `npm run dev` / `npm run dev:management` / `npm run dev:backend`
- `npm run build` / `npm run build:management`
- `npm run test` / `npm run test:management` / `npm run test:backend`
...
| 文档                                                                     | 内容                               |
| ------------------------------------------------------------------------ | ---------------------------------- |
| [README.md](../README.md)                                                | 安装、联调、健康检查、API Key 说明 |
| [api-response-conventions.md](./api-response-conventions.md)             | JSON 响应形态与统一化方向          |
| [management-api-troubleshooting.md](./management-api-troubleshooting.md) | 管理端连不上 API 的排查            |
| [site-api-env.md](./site-api-env.md)（若存在）                           | 站点与 API 环境变量                |
| [mgmt-crud-verification.md](./mgmt-crud-verification.md)（若存在）       | 管理 CRUD 验证说明                 |

---

## 14. 文档维护说明

- 架构变更（新子应用、新网关、鉴权方式变更）时，请同步更新 **第 4～7 节** 与 Mermaid 图。
- 新增重要 `/api` 资源时，建议在 **第 6 节** 补充命名前缀，并在契约测试中固化最小响应形状。

---

_文档版本与仓库一致；最后更新以 Git 提交记录为准。_
